...
const AnalyticsDashboard: React.FC = () => {
  ...
  const calculateCompletionRate = (leads: any[]) => {
    const completedLeads = leads.filter(lead => 
      lead.status === 'Completed' || lead.status === 'Sold'
    ).length;
  ...
  const processTrendsData = (interactions: any[]) => {
    ...
    return Object.values(dailyData).map((item: any) => ({
      ...item,
      uniqueLeads: item.uniqueLeads.size
    }));
  };
  ...
  const processTopPerformers = (interactions: any[]) => {
    ...
    return Object.values(performers)
      .map((performer: any) => ({
        ...performer,
        uniqueLeads: performer.uniqueLeads.size
      }))
    ...
    <Box>
      ...
      <Paper sx={{ width: '100%' }}>
        ...
        <TabPanel value={tabValue} index={0}>
          <Typography variant="h6" gutterBottom>Interaction Trends</Typography>
        ...
        <TabPanel value={tabValue} index={1}>
          <Typography variant="h6" gutterBottom>Interaction Outcomes</Typography>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
